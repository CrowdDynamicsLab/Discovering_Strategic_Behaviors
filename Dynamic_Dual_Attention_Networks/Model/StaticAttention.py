...
class StaticAttentionLayer(nn.Module):
    
    def __init__(self, in_dim, out_dim, alpha, device):
        super(StaticAttentionLayer, self).__init__()
    ...
    def forward(self, a_feats, b_feats, b_dis, adj_ab):
        ...
        new_a_dis = torch.matmul(attention, b_dis)
        new_a_dis = torch.tanh(new_a_dis)
        new_a_dis = F.normalize(new_a_dis,p=1,dim=-1)
        
        return new_a_dis, attention.detach()
    

class StaticAttentionModule(nn.Module):
    
    def __init__(self, in_dim, out_dim, alpha, device):
        super(StaticAttentionModule, self).__init__()
        
        self.att_layer = StaticAttentionLayer(in_dim, out_dim, alpha, device).to(device)
        
    def forward(self, batch_input):
        
        batch_a_feats, batch_b_feats, batch_trans_adj, batch_b_dis = batch_input

        new_a_dis, batch_att = self.att_layer(batch_a_feats, batch_b_feats, batch_b_dis, batch_trans_adj)
        
        batch_att = batch_att[batch_trans_adj[0], batch_trans_adj[1]]        
        return new_a_dis, batch_att